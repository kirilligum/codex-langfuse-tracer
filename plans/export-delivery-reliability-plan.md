...
- **Availability remains unaccepted (2026-09-22 14:26 PDT).** Systemd recorded repeated OOM kills of processes in `codex-langfuse-watch.service`; its restart counter rose from 2 during the initial check to 32. It was active/running at the 14:26 snapshot (PID 333811), but that point-in-time status does not establish availability. Unit logs show `status=9/KILL`, `Result=oom-kill`, and several recent per-run peaks of 35.5-153.6 MiB; earlier OOM snapshots showed watcher RSS around 0.77-0.79 GiB. The host snapshot showed 30 GiB RAM, 29 GiB used, 197 MiB free, 1.1 GiB available, 8 GiB swap effectively full (208 KiB free), and memory PSI `some` avg10/60/300 of 68.09/47.76/70.48 and `full` of 40.55/29.31/44.22. Nearby process snapshots included large `workerd`, Codex, Node/web-server, and ClickHouse processes. These observations establish severe host pressure and watcher OOM victimization, not which workload caused the pressure. No unrelated process was stopped and no OOM priority, cgroup limit, or service restart policy was changed.
...
| P5 publication, install, and deployment | Runtime revision `78bfafeeb27f619e4df6f9af01cb768c9dacc55c` on `origin/main` | `./install.sh`; pre/post doctor; version 3 state; installed and running executable SHA-256 `a7cb113f601832f3faf7fdcdc927c71b0e29ff8ed4deac6bdad301a2547f9e93`; build VCS revision matches `78bfafe`; Codex automatic canary followed by read-only `TestLiveCodexSmokeTrace` | **PARTIAL / availability blocked.** Publication, installation, state preservation, and canary shape passed: one root, one transcript, two total observations, unique IDs, stable for 5 seconds; canary trace is processed. Claude CHECK-001 is unperformed because the CLI is absent. Subsequent monitoring found repeated systemd-recorded OOM kills and `NRestarts=32` at 14:26 PDT despite the service being active at that instant. Host RAM/swap/PSI remain severely pressured; initiating workload is unknown. Complete the recovery checks above before claiming production availability. |
