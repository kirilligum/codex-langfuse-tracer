...
- Status: P0-P6 implementation, tests, publication, and installation are complete. Runtime code `0912de5c89d93f0edc8f0b43217c2bc4fea57882` is installed in the binary built at `03139fa58eb57c9a66b24c76903847d9c1330ea3`; the 117,007,638-byte rollout probe passed at 41,408 KiB maximum RSS. The post-install 60-minute observation and a fresh automatic Codex canary passed: zero watcher restarts, no kernel OOM events, healthy doctor/state, and one root plus one transcript stable for five seconds. Available RAM and memory PSI recovered; the host's 8 GiB swap file remained full. Treat the unknown host-wide OOM source and full swap as residual operational risks; do not reclaim swap or interrupt unrelated workloads without owner review.
...
5. Deploy the new revision without resetting v3 state or manually replaying a trace. Verify installed/running digests, doctor/state, and watcher RSS/restart count while scanning the same eligible source. Require at least 60 minutes with no watcher restart or OOM event, active/running status, at least 4 GiB `MemAvailable`, and low memory PSI. Record swap use and swap-in/out activity. A nonzero `SwapFree` value is not a standalone gate when physical memory headroom is sustained and PSI/paging activity show no active pressure; never run `swapoff` or stop unrelated workloads to manufacture swap headroom. Then run one fresh automatic canary and its read-only shape check.
...
- **Recovery/acceptance checks:** identify the lifecycle and budget of large active memory consumers before choosing any workload-specific remediation; do not interrupt work without owner review. Record host/service snapshots across a 60-minute window. Acceptance requires no increase in `NRestarts`, no watcher OOM events, sustained active/running status, at least 4 GiB `MemAvailable`, low memory PSI and no sustained swap activity, clean `--doctor`, intact version 3 state with no unexpected pending/queued work, and matching installed/running executable digests. Record raw swap use; do not require `SwapFree` to be nonzero when available physical memory and pressure measurements demonstrate headroom, and do not force swap reclamation. Then send one fresh benign turn through one automatic path and run the provider-matched read-only smoke check; do not manually replay it. A shorter healthy interval is not evidence that sustained availability is restored.
