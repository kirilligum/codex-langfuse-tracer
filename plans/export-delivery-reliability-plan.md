...
- Status: P0-P4 complete. Runtime revision `78bfafeeb27f619e4df6f9af01cb768c9dacc55c` is published and installed, and the canary shape check passed. P5 production availability acceptance is pending after repeated host OOM kills. P6 memory reduction is implemented and local tests pass; commit, installation, and production observation remain pending.
...
| P5 publication, install, and deployment | Runtime revision `78bfafeeb27f619e4df6f9af01cb768c9dacc55c` on `origin/main` | `./install.sh`; pre/post doctor; version 3 state; installed and running executable SHA-256 `a7cb113f601832f3faf7fdcdc927c71b0e29ff8ed4deac6bdad301a2547f9e93`; build VCS revision matches `78bfafe`; Codex automatic canary followed by read-only `TestLiveCodexSmokeTrace` | **PARTIAL / availability pending.** Publication, installation, state preservation, and canary shape passed: one root, one transcript, two total observations, unique IDs, stable for 5 seconds; canary trace is processed. Claude CHECK-001 is unperformed because the CLI is absent. At 14:36 PDT the active service had `NRestarts=49`; recent journal entries through restart 45 confirm repeated OOM kills. At 15:09 the old revision remained active with `NRestarts=85` (unchanged since 15:03), 4.6 GiB available RAM, full swap, and recovered short PSI averages. This is not the required post-install 60-minute acceptance. The triggering workload remains unidentified; P6 addresses the measured watcher memory amplifier. |
| P6 bounded-memory Codex scan | Working tree based on `b895219` | 2,000-turn filtered parser regression; watcher-level processed-turn regression; parse-error watermark regression; `GOMAXPROCS=1 go test -p=1 ./... -count=1`; `GOMAXPROCS=1 go test -race -p=1 ./internal/codextrace ./internal/watch -count=1`; 10-second `FuzzParseTurnsDoesNotPanic`; `git diff --check` | **PASS locally; deploy pending.** JSONL reads line-by-line; watcher creates a processed-ID set after draining queued work and filters completed traces before retaining their observations; parse failures leave the prior watermark intact. The real eligible 114.9 MB rollout has a 4.48 MB maximum line. Installation, memory measurement on that rollout, and post-install 60-minute availability checks remain outstanding. |
