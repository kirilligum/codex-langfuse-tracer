...
The next planning step is to define the desired delivery behavior for acceptance/checkpoint ambiguity and multiple writers, using the deployed lock correction as the baseline. Reconciliation and score migration are separate work unless the resulting delivery contract requires them.
