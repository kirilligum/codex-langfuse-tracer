...
- Compute controls:
  ...
  - early_stop%: suspend a phase when estimated phase confidence is below 85% after RED triage or feature creep is above 25%.
...
| Pricing | DECISION | Claude model pricing is added only after source-checking official Anthropic pricing during implementation. No local cost multiplication, fallback aliases, or unpriced Claude model guesses are allowed. |
...
- At least three synthetic Claude fixtures exist in the single manifest: no-tools final answer, Bash or tool-use turn, and incomplete or corrupt transcript.
...
- Update `README.md`, `TESTING.md`, `AGENTS.md`, `testdata/manifest.json`, fixture sources, and golden contract tests.
...
- Pricing support can double count cache usage if usage buckets are mapped incorrectly.
- Docs can claim support before the parser, CLI, hook, and projection paths are tested.

### Assumptions

- Synthetic fixtures are acceptable for committed Claude transcripts; real local transcript content is not committed.
...
- REQ-001 type func: Parse Claude Code transcript JSONL into exportable turns.
  - Acceptance: parser accepts synthetic Claude JSONL with user, assistant, tool_use, tool_result, model, usage, cwd, and session fields; incomplete turns are non-exportable.
...
- REQ-007 type func: Map Claude tool calls into canonical observations.
  - Acceptance: Bash, Read, Edit, Write, MultiEdit, WebSearch, WebFetch, MCP-like tools, Agent-like tools, and unknown tools map to stable families and bounded metadata.
- REQ-008 type func: Map model and token usage without local cost calculation.
  - Acceptance: Claude model and usage details are emitted only when present; pricing definitions are source-dated before being added.
...
- REQ-010 type func: Document real Claude support level.
  - Acceptance: README, TESTING, and AGENTS describe only implemented behavior and keep install/use wording concise.
...
- REQ-026 type data: Keep pricing catalog source-backed.
  - Acceptance: every Claude model pricing entry records a source URL, source date, and explicit model ID; missing pricing omits cost details rather than guessing.

### Error handling and telemetry expectations

- Parser errors include source path and line number for JSONL files.
- Unknown Claude record types are ignored only when they are not needed for visible conversation export.
- Unknown Claude tools emit a generic tool observation with a normalized name and safe metadata.
...
| Claude schema drift | Local Claude transcripts include record shapes absent from synthetic fixtures | Keep parser tolerant of unknown non-conversation records and add fixture-backed support only for observed export fields. |
...
| Pricing errors | Claude cache buckets are double counted | Add pricing tests before catalog changes and rely on Langfuse model definitions. |
...
- Steps:
  ...
  - Step 2 GREEN: update `testdata/manifest.json`, move Codex fixture sources to `testdata/sources/codex`, add synthetic Claude sources under `testdata/sources/claude`, and update test loaders; run TEST-501 with `go test ./test -run TestFixtureManifestProviderSources -count=1`; expected PASS.
...
- Exit gates:
  - Green criteria: manifest has provider, source, source_format, golden, and categories for every fixture.
...
- Steps:
  ...
  - Step 2 GREEN: update Codex parser and tracecontract adapters to return and consume `agenttrace` values; run TEST-503 with `go test ./internal/codextrace ./test -run 'TestCodexParserUsesAgentTrace|TestGoldenTraceContract' -count=1`; expected PASS.
...
- Scope and objectives:
  ...
  - Add `internal/claudetrace` parser for synthetic Claude Code transcript JSONL.
...
- Steps:
  ...
  - Step 2 GREEN: implement line-by-line Claude transcript parsing, turn segmentation, tool_use to tool_result pairing, model and usage extraction, thinking omission, redaction, deterministic IDs, and parse errors; run TEST-504 with `go test ./internal/claudetrace -count=1`; expected PASS.
...
- Exit gates:
  - Green criteria: no-tools, tool-use, incomplete, corrupt, and thinking fixtures parse deterministically.
  - Yellow criteria: parser handles fixtures but tool family metadata is too sparse for Langfuse projection.
...
- Steps:
  - Step 1 RED: create/update TEST-506 in `cmd/codex-langfuse-exporter/cli_test.go` and `cmd/codex-langfuse-exporter/main_integration_test.go` for REQ-004, REQ-014, REQ-021, and REQ-022; run `go test ./cmd/codex-langfuse-exporter -run 'TestCLIProviderSelection|TestManualClaudeExportCLIIntegration' -count=1`; expected FAIL because `--provider claude` does not exist.
  - Step 2 GREEN: add `--provider codex|claude`, default to Codex, route Claude `--path` through `internal/claudetrace`, reject Claude `--latest` and `--session-id`, and reuse `--config`; run TEST-506 with `go test ./cmd/codex-langfuse-exporter -run 'TestCLIProviderSelection|TestManualClaudeExportCLIIntegration' -count=1`; expected PASS.
  - Step 3 REFACTOR: split provider dispatch into small functions without adapters or fallback parsing; run TEST-599 with `go test ./... -count=1`; expected PASS.
...
- Phase metrics:
  ...
  - Internal interactions: 4, `cmd`, `internal/codextrace`, `internal/claudetrace`, `internal/config`.
...
### Phase P07: Docs, pricing, and public contract

- Scope and objectives:
  - Impacted requirements: REQ-008, REQ-010, REQ-023, REQ-026.
  - Update public docs after code behavior exists.
  - Add Claude pricing only with source-dated official pricing.
- Restore points:
  - Before RED: `git tag -f clt-claude-P07-start HEAD`
  - After exit gates: `git tag -f clt-claude-P07-green HEAD`
- Steps:
  - Step 1 RED: create/update TEST-508 in `test/docs_static_test.go` and `internal/langfuse/models_test.go` for REQ-008, REQ-010, REQ-023, and REQ-026; run `go test ./... -run 'TestDocsClaudeSupportContract|TestAnthropicPricingSourceDated' -count=1`; expected FAIL because README lacks Claude support docs and Claude pricing entries are absent.
  - Step 2 GREEN: update README, TESTING, AGENTS, and model pricing definitions only for source-checked Anthropic model IDs; run TEST-508 with `go test ./... -run 'TestDocsClaudeSupportContract|TestAnthropicPricingSourceDated' -count=1`; expected PASS.
  - Step 3 REFACTOR: remove duplicated docs wording and keep the active docs contract in README, TESTING, and AGENTS; run TEST-599 with `go test ./... -count=1`; expected PASS.
  - Step 4 MEASURE: run EVAL-008 with `go test ./test -run TestEvalDocsClaudeContractCompleteness -count=1`; expected thresholds met.
- Exit gates:
  - Green criteria: docs describe only parser-tested, CLI-tested, hook-queued Claude support.
  - Yellow criteria: docs are correct but too long or duplicate TESTING content.
  - Red criteria: docs claim native Claude OTEL forwarding, Claude polling, or automatic settings mutation.
- Phase metrics:
  - Confidence %: 82, because docs tests can guard public claims.
  - Long-term robustness %: 86, because source-dated pricing reduces future ambiguity.
  - Internal interactions: 5, README, TESTING, AGENTS, `test/docs_static_test.go`, `internal/langfuse/models.go`.
  - External interactions: 1, official Anthropic pricing page during implementation.
  - Complexity %: 38.
  - Feature creep %: 20.
  - Technical debt %: 5.
  - YAGNI score: 10.
  - MoSCoW: Should.
  - Local/non-local scope: docs and pricing catalog.
...
- Scope and objectives:
  ...
  - Keep optional live Claude smoke outside RTM.
- Restore points:
...
- Steps:
  ...
  - Step 2 GREEN: connect parser, CLI, hook queue, watch drain, Langfuse projection, fixture contract, docs, and pricing behavior; run TEST-511 and TEST-512 with `go test ./test -run 'TestFullClaudeAcceptance|TestNoLegacyDuplicateClaudePaths' -count=1`; expected PASS.
...
- Exit gates:
  ...
  - Yellow criteria: all tests pass but optional live smoke has not been executed.
...
- Phase metrics:
  ...
  - External interactions: 0 for RTM, 2 for optional live smoke with Claude Code and Langfuse.
  - Complexity %: 52.
  - Feature creep %: 8.
  - Technical debt %: 4.
  - YAGNI score: 6.
...
- id: TEST-504
  ...
  - fixtures/mocks/data: `testdata/sources/claude/no-tools.jsonl`, `testdata/sources/claude/tool-use.jsonl`, `testdata/sources/claude/incomplete.jsonl`, `testdata/sources/claude/corrupt.jsonl`, `testdata/sources/claude/thinking.jsonl`
  - deterministic controls: no network, fixed synthetic transcript JSONL, repeated parse comparison, parser runtime budget 200ms for fixture corpus
  - pass_criteria: parser returns expected exportable turns, pairs tool_use/tool_result IDs, omits thinking blocks, redacts sentinel secrets, includes path and line in corrupt JSON errors, and derives provider-separated IDs
  - expected_runtime: 5s
...
- id: TEST-506
  ...
  - command: `go test ./cmd/codex-langfuse-exporter -run 'TestCLIProviderSelection|TestManualClaudeExportCLIIntegration' -count=1`
  - fixtures/mocks/data: synthetic Claude transcript fixture, temporary Langfuse config, fake HTTP server
...
- id: TEST-507
  ...
  - fixtures/mocks/data: hook JSON fixtures, temp queue file, synthetic Claude transcript fixture, fake export callback
...
- id: TEST-508
  - name: Claude docs and Anthropic pricing contract
  - type: static
  - verifies: REQ-008, REQ-010, REQ-023, REQ-026
  - location: `test/docs_static_test.go`, `internal/langfuse/models_test.go`
  - command: `go test ./... -run 'TestDocsClaudeSupportContract|TestAnthropicPricingSourceDated' -count=1`
  - fixtures/mocks/data: `README.md`, `TESTING.md`, `AGENTS.md`, `internal/langfuse/models.go`
  - deterministic controls: no network in tests, source URL/date literals in model catalog
  - pass_criteria: docs describe only implemented Claude support; unsupported claims are absent; every Claude model pricing entry has source URL/date and no local cost math
...
- id: TEST-511
  ...
  - fixtures/mocks/data: synthetic Claude fixtures, Codex fixtures, fake Langfuse server, temp queue/state
...
- id: CHECK-001
  - name: Live Claude hook smoke before removing experimental wording
  - procedure:
    - Build and install using the repository install flow after TEST-599 and TEST-598 pass.
    - Add the README hook snippet to `~/.claude/settings.json`.
    - Run a harmless Claude Code prompt that does not read private files: `Reply exactly: claude-langfuse-smoke-test`.
    - Confirm Langfuse shows one trace named `claude.turn.transcript` with non-empty input and output and no thinking content.
  - exclusion from RTM: this depends on local Claude Code, user settings, and live Langfuse credentials.
...
- Committed Claude fixtures are synthetic and secret-free.
- Real Claude transcript bodies are not committed.
...
- Seeds:
  ...
  - Synthetic Claude fixtures under `testdata/sources/claude`.
...
| P05 | REQ-004 | TEST-506 | `cmd/codex-langfuse-exporter/cli_test.go`, `cmd/codex-langfuse-exporter/main_integration_test.go` | `go test ./cmd/codex-langfuse-exporter -run 'TestCLIProviderSelection|TestManualClaudeExportCLIIntegration' -count=1` |
...
| P07 | REQ-008 | TEST-508 | `test/docs_static_test.go`, `internal/langfuse/models_test.go` | `go test ./... -run 'TestDocsClaudeSupportContract|TestAnthropicPricingSourceDated' -count=1` |
| P00 | REQ-009 | TEST-501 | `test/contract_fixture_test.go` | `go test ./test -run TestFixtureManifestProviderSources -count=1` |
| P07 | REQ-010 | TEST-508 | `test/docs_static_test.go`, `internal/langfuse/models_test.go` | `go test ./... -run 'TestDocsClaudeSupportContract|TestAnthropicPricingSourceDated' -count=1` |
...
| P05 | REQ-014 | TEST-506 | `cmd/codex-langfuse-exporter/cli_test.go`, `cmd/codex-langfuse-exporter/main_integration_test.go` | `go test ./cmd/codex-langfuse-exporter -run 'TestCLIProviderSelection|TestManualClaudeExportCLIIntegration' -count=1` |
...
| P05 | REQ-021 | TEST-506 | `cmd/codex-langfuse-exporter/cli_test.go`, `cmd/codex-langfuse-exporter/main_integration_test.go` | `go test ./cmd/codex-langfuse-exporter -run 'TestCLIProviderSelection|TestManualClaudeExportCLIIntegration' -count=1` |
...
| P05 | REQ-022 | TEST-506 | `cmd/codex-langfuse-exporter/cli_test.go`, `cmd/codex-langfuse-exporter/main_integration_test.go` | `go test ./cmd/codex-langfuse-exporter -run 'TestCLIProviderSelection|TestManualClaudeExportCLIIntegration' -count=1` |
...
| P07 | REQ-023 | TEST-508 | `test/docs_static_test.go`, `internal/langfuse/models_test.go` | `go test ./... -run 'TestDocsClaudeSupportContract|TestAnthropicPricingSourceDated' -count=1` |
...
| P07 | REQ-026 | TEST-508 | `test/docs_static_test.go`, `internal/langfuse/models_test.go` | `go test ./... -run 'TestDocsClaudeSupportContract|TestAnthropicPricingSourceDated' -count=1` |
...
- ADR-006: Claude pricing entries require source URL, source date, and explicit model IDs.
